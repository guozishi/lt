...
<head>
	...
	<script type="text/javascript" async="" src="about:blank"></script><script async="" charset="utf-8" src="about:blank"></script>
...
<div class="wrap">
	...
	<div class="bd" id="middle">
		...
		<!--登录框 begin-->
		  <div class="area">
		    <div class="login-area" id="login_area"><div class="TY-login-window" id="TY_login_window_1504615577480"><div class="login-window-body"><div id="loginWin_content_wrapper" class="loginWin-normal-login-wrapper"><div style="font-size:40px;font-weight:bold;font-family: '楷体';text-align:center;">登录</div><div class="loginWin-form">
			...
			{{ csrf_field() }}
				<input class="input-text" id="vwriter" name="name" placeholder="请输入用户名" autocomplete="off" type="text">&nbsp;&nbsp;&nbsp;<span style="color: red;"></span><input id="user_action" name="action" value="f0.1504615577602.0,,,,b1.5.6209|bcdc8ed3fea93607445162a5a180ef5d|d41d8cd98f00b204e9800998ecf8427e|Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:55.0) Gecko/20100101 Firefox/55.0|v2.2" type="hidden"></div><div class="loginWin-form-item loginWin-password-input">
			<input class="input-text" id="vpassword" name="password" placeholder="请输入密码" type="password">&nbsp;&nbsp;&nbsp;<span style="color: red;"></span></div><div class="loginWin-form-item fs12 cf"><label class="fl"><input class="input-checkbox" id="rmflag" name="rmflag" value="1" checked="checked" type="checkbox">下次自动登录</label><a class="loginWin-password-link fr" href="http://passport.tianya.cn/fp/fp.jsp" target="_blank">忘记密码？</a></div><div class="loginWin-form-item"><button id="btn" type="submit" class="loginWin-submit-btn">登 录</button></div><div class="fs12 cf"><a class="loginWin-register-link fr" href="{{ url('home/register') }}" target="_blank">立即注册</a></div><input name="__sid" value="1#3#1.0#e82115bf-74fe-40f2-a0fa-a559c8bae8f3" type="hidden"><input name="fowardURL" value="" type="hidden"><input name="returnURL" value="" type="hidden"></form><dl class="loginWin-third-list cf"><dt><div class="loginWin-qrcode"><div class="loginWin-qrcode-box"><img class="loginWin-qrcode-img" src="htm.dat"><div class="loginWin-qrcode-tips"></div></div></p></div></div></div></div></div>
            <div class="onLine-num">
                 <span class="fl online"> 当前在线：<em id="spanOnLine">885419</em></span>
                 <span class="fr"><a href="{{ url('home/index/index') }}" class="u_line">浏览进入&gt;&gt;</a></span>
            </div>
		  </div>
		<!--登录框 end-->
	    </div>
...
<script type="text/javascript">
    ...
    $("#vwriter").blur(function() {
        var v=$(this).val();
        if (v=='') {
            $("#vwriter").next().html("姓名不能为空！");
            $(this).css("color","#f00");
        }else if(!v.match(h)){
            $("#vwriter").next().html("姓名不合法！");
            $("#vwriter").css("color","#f00");
        }else{
    ...
    $("#vpassword").blur(function() {
        ...
        if (v=='') {
            ...
            $("#vpassword").next().html("密码不能为空！");
            $(this).css("color","#f00");
        }else if(!v.match(p)){
            $("#vpassword").next().html("请填写正确的密码！");
            $("#vpassword").prev().css("color","#f00");
        }else{
            $(this).prev().css("color","#0EA74A");
            $("#vpassword").next().html("");
        }
    ...
    $('.submit').click(function(){
        var name=$("#vwriter").val();
        var password=$("#vwriter").val();
        if (name=="") {
            $("#vwriter").next().html("姓名不能为空！");
            return;
        }else if(!name.match(h)){
            $("#vwriter").next().html("姓名不合法！");
            $("#vwriter").prev().css("color","#f00");
            return;
        }

        if (password!='' && !password.match(p)) {
            $("#vpassword").next().html("请填写正确的密码！");
            $("#vpassword").prev().css("color","#f00");
        }
    });
