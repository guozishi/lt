      ...
      <table class="table table-striped b-t b-light">
        ...
        @foreach( $coms as $item )
          <tr>
            ...
            <td>{{ $item->art_id }}</td>
            <td>
                <a class="glyphicon glyphicon-align-left" href="javascript:;" onclick="detail({{ $item->art_id }})">详情</a>&nbsp;&nbsp;
                <a class="label bg-success" href="javascript:;" onclick="sendBack({{ $item->cmpl_id }})">退回</a>&nbsp;&nbsp;
                <a class="label bg-danger" href="javascript:;" onclick="del({{ $item->art_id }})">删除原贴</a>
            </td>
            <td>
...
//        alert(num);
    ...
    function sendBack(cmpl_id) {
//        询问框
        layer.confirm('<b>您要退回吗？</b>'+'<br>'+'(注:若退回将降低投诉者阳光投诉星级)', {
            btn: ['确定','再看看'] //按钮
        }, function() {
                $.post('{{url('/admin/complain/sendBack')}}/' + cmpl_id, {'_token': '{{ csrf_token() }}'}, function (data) {
                    if(data.state==0){
      ...
      function del(art_id){
          layer.confirm('<b>您确定要删除吗？</b>', {
              ...
              $.post('{{ url('/admin/cmplain/del') }}/'+art_id,{ '_token':'{{ csrf_token() }}'},function (data) {
                  if (data.state == 0) {
                      layer.msg(data.msg, {icon: 1});
                      setTimeout(function () {
                          location.href = location.href;
