      ...
      if (Md5($data['password']) != $res->password)
      {
       ...
       //退出登录
        ...
        public function logout(Request $request)
        {
            ...
            return redirect('/home/login')->with(['info.success'=>'退出成功!']);
        }
        ...
        //注册用户
         public function register(Request $request){
          if($request->isMethod('post')){
            ...
            if ($cookie)
             {
                ...
                $data = $request->except('_token');
                $da = $request->all();
                $da = $request -> except('password','_token');
            ...
            {
                  $request->flash();
                  return back() -> with(['info.error'=>'用户已存在,请使用其他用户名进行注册']);
            }else{


                 \DB::table('user_login')->insertGetId($data);
                \DB::table('users_info')->insert($da);
                 
                  return redirect('/home/login')->with(['info.success'=>'您的账户已经注册成功!']);
            }
              }
